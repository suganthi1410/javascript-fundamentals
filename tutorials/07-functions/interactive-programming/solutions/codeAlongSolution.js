...
function loadSupplies() {
    ...
    if (chosenSector[2] + validAmount > MAX_SECTOR_CAPACITY) {
        ...
        console.log(`Maximum limit is ${MAX_SECTOR_CAPACITY}t. You only have room for ${availableSpace}t.`);
    } else if (chosenSector[1] === "fuel" && validAmount > 100) {
        // Special situational conditional restriction
        printFormattedHeader("SAFETY WARNING: Volatile fuel intake restricted to 100t per intake cycle.", true);
    } else {
        // Successfully update mutable data structures
        chosenSector[2] += validAmount;
        console.log(`Success! ${validAmount}t added to ${chosenSector[0]}. New total: ${chosenSector[2]}t.`);
    }
}
